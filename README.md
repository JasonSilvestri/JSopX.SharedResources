...
### Phase 1 of 4
Create the initial `JSopX.SharedResources` Project (the React & Asp.NET Core Web API Demo application). 

## Overview
The `JSopX.SharedResources` Project (React & Asp.NET Core Web API Demo application) which leverages the same assets, components & Web API all other projects in the JSopX™ Open Project EXperiences collection, and is the official React Project Example of the Enterprise-Level, Multi-Platform, Multi-Stack Application. 

### Prerequisites

The `JSopX.SharedResources` Project requires at least the following technologies to run as expected.
...
There are additional project resources from the [jSilvestri.com BETA v 2024 JSopX™ Open ProjectX Collection](https://github.com/JasonSilvestri/JSopX.OpenProjectX.git) that required in order for the The `JSopX.SharedResources` Project to run as expected. 
...
2. **Open the solution in Visual Studio**:
    ...
    - Open `JSopX.SharedResources.sln` in Visual Studio.
